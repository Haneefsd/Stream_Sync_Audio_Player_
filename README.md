...
* 📑 **Dedicated Full-Page Playlist Views:** Playlists open as comprehensive standalone pages (`PlaylistView.jsx`) with custom cover uploads, 1st-song artwork fallback, track reordering, and in-page song search.
...
* 📲 **Official StreamSync App Icon & Full PWA Support:** Features the signature emerald-to-cyan gradient squircle (`#10b981` to `#06b6d4`) with the centered black broadcast/radio waves mark `(( · ))`. Unified across all platforms with clean vector SVG (`/icon.svg`), multi-size Windows icon (`/favicon.ico`), Apple Touch Icon (`/apple-touch-icon.png`), and Android/PWA icons (`/pwa-192x192.png`, `/pwa-512x512.png`, `/maskable-icon-512x512.png`). 100% PWA compliant (`vite-plugin-pwa`) with automatic Service Worker offline app shell caching, standalone window installation for desktop/mobile, and OS-level **MediaSession API** integration for lock screen and Bluetooth playback controls.
